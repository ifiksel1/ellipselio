...
//
// ROS2 port: namespace photometric, no ros:: / ROS_ / cv_bridge dependencies.
// Algorithm is numerically identical to the ROS1 original.
...
FeatureManager::FeatureManager(std::shared_ptr<Projector> projector,
                               int margin,
                               double min_range,
                               double max_range,
                               double grad_min,
    ...
    : projector_(projector),
      ...
      thr_gradmin_(grad_min),
      ncc_threshold_(ncc_threshold),
      ...
      n_added_last_(0)
{
    if (std::find(feature_modes.begin(), feature_modes.end(), mode_) == feature_modes.end()) {
        std::cerr << "[FeatureManager] Invalid mode, setting to complementary\n";
        mode_ = "comp";
    }
    if (patch_size_ % 2 == 0) {
        std::cerr << "[FeatureManager] Patch size must be odd, setting to 5\n";
        patch_size_ = 5;
    }
...
void FeatureManager::detectFeatures(const LidarFrame& frame, const std::vector<V3D>& V, const M4D& T_GL) {
    ...
    if (mode_ == "comp") {
        ...
        std::cerr << "[FeatureManager] Invalid mode\n";
        return;
    }
    ...
    for (size_t i = 0; i < features_uv.size(); i++) {
        ...
        for (size_t l = 0; l < static_cast<size_t>(patch_idx_.rows()); l++) {
            const int point_idx = frame.img_idx.ptr<int>(features_uv[i].y + patch_idx_(l,1))[features_uv[i].x +
                patch_idx_(l,0)];

            // Sparse splat: patch pixels without a real point (img_idx==-1) are skipped
            // so the patch only carries valid LiDAR returns (v1 with downsampled cloud).
...
void FeatureManager::detectFeaturesComp(const LidarFrame& frame, const std::vector<V3D>& V, const int n_features,
    ...
    #endif
    for (size_t v = 0; v < static_cast<size_t>(frame.img_mask.rows); v++) {
        for (size_t u = 0; u < static_cast<size_t>(frame.img_mask.cols); u++) {
    ...
    for (size_t v = 0; v < static_cast<size_t>(grad_img.rows); v++) {
        for (size_t u = 0; u < static_cast<size_t>(grad_img.cols); u++) {
    ...
    for (size_t idx = 0; idx < sorted_indices.size(); idx++) {
        ...
        if (features_uv.size() >= static_cast<size_t>(n_features)) {
            break;
        }
    }
}
...
void FeatureManager::detectFeaturesRandom(const LidarFrame& frame, const int n_features,
    ...
    while (candidates.size() < static_cast<size_t>(n_features) && valid_locations.size() > 0) {
...
void FeatureManager::removeFeatures(const std::vector<int>& idx) {
    ...
    for (size_t i = 0; i < features_.size(); i++) {
        if (std::find(idx.begin(), idx.end(), static_cast<int>(i)) == idx.end()) {
...
bool FeatureManager::IsFov(const cv::Point2f& pt) const {
